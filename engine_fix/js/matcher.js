...
async function loadVendorItems() {
    try {
        ...
        showToast('⚠️ Could not load vendor catalog', 'warning');
...
// Extract NUPCO codes from Excel data
function extractNUPCOCodes(data) {
    ...
    // If no header found, assume first column has codes
    if (codeColumnIndex === -1) {
        codeColumnIndex = 0;
        headerRowIndex = 0;
    }
    
    // Extract codes from data rows
    for (let i = headerRowIndex + 1; i < data.length; i++) {
        const row = data[i];
        if (row[codeColumnIndex]) {
            const code = String(row[codeColumnIndex]).trim();
            // Validate code format (contains numbers and possibly dashes)
            if (code && /\d/.test(code)) {
                items.push(code);
            }
        }
...
function matchItems(nupcocCodes) {
    matchedItems = [];
    notFoundItems = [];
    
    nupcocCodes.forEach(code => {
        // Find matching vendor item
        const vendorItem = vendorItems.find(item => 
            item.nupco_code === code || 
            item.nupco_code === code.replace(/^0+/, '') || // Try without leading zeros
...
// Get price history for a NUPCO code
function getPriceHistory(nupcoCode) {
    // Find most recent price entry for this code
    const entries = priceHistory.filter(entry => entry.NUPCO_Code === nupcoCode);
...
function displayMatchedItems() {
    ...
    matchedItems.forEach((item, index) => {
        ...
        row.innerHTML = `
            ...
            <td class="py-4 px-4 text-sm text-gray-300">${item.pack}</td>
            <td class="py-4 px-4">
...
function displayNotFoundItems() {
    ...
    notFoundItems.forEach(code => {
        ...
        item.className = 'bg-red-500/10 border border-red-500/30 rounded-lg p-3';
        item.innerHTML = `
            <p class="text-sm font-mono text-red-400">${code}</p>
        `;
